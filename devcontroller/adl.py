...
class ADLController(Loggable):
    ...
    def __init__(self, sputter, logger):
        super(ADLController, self).__init__(logger)
        assert isinstance(sputter, ADLSputterDriver)

        self._driver = sputter
    ...
    @retry(retry_count=2)
    def initialize(self):
        coeff = self._driver.get_coefficients()
    ...
    def sputter(self, value, mode=ADLSputterDriver.MODE_POWER):
        ...
        self._driver.set_mode(mode, value, True, coeff=coeff)
        self.turn_on()

    @retry()
    def sputter_power(self, power):
        self.__check_mode(ADLSputterDriver.MODE_POWER)
        power = self._driver.convert_into_power(power, coeff=self.coeff_power)
        self._driver.clear()
        self._driver.set_mode_p(power)

    @retry()
    def sputter_voltage(self, voltage):
        self.__check_mode(ADLSputterDriver.MODE_VOLTAGE)
        voltage = self._driver.convert_into_voltage(voltage, coeff=self.coeff_volt)
        self._driver.clear()
        self._driver.set_mode_u(voltage)
    ...
    def turn_on(self):
        if self.thread is None or not self.thread.is_running():
            ...
            self.thread.set_driver(self._driver, self._logger)
            self.thread.start()
        else:
    ...
    def get_actual_values(self):
        return self._driver.get_actual_value()

    def get_power(self):
        return self._driver.convert_from_power(self.get_actual_values().get_power(), coeff=self.coeff_power)

    def get_voltage(self):
        return self._driver.convert_from_voltage(self.get_actual_values().get_voltage(), coeff=self.coeff_volt)

    def get_current(self):
        return self._driver.convert_from_current(self.get_actual_values().get_current(), coeff=self.coeff_current)
